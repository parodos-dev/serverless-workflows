...
out=$(curl -XPOST -H "Content-Type: application/json" -H "Authorization: Bearer ${BACKEND_SECRET}" \
    http://localhost:9080/api/orchestrator/v2/workflows/mta-analysis-v6/execute -d '{"inputData": {"repositoryURL": "https://github.com/spring-projects/spring-petclinic", "recipients": ["user:default/guest"], "exportToIssueManager": "false", "migrationStartDatetime" : "2024-07-01T00:00:00Z", "migrationEndDatetime" : "2024-07-31T00:00:00Z"}}')
id=$(echo "$out" | jq -e .id)
