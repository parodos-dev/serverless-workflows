...
yq --inplace '.spec.podTemplate.container.env |= ( . + [{"name": "QUARKUS_REST_CLIENT_MTA_JSON_URL", "value": "http://tackle-ui.my-konveyor-operator.svc:8080"}, {"name": "BACKSTAGE_NOTIFICATIONS_URL", "value": "http://janus-idp-workflows-backstage.default.svc.cluster.local:7007/api/notifications/"}] )' manifests/04-sonataflow_mta-analysis-v6.yaml

# Disable persistence for e2e tests
yq e '.spec.persistence = {}' -i manifests/04-sonataflow_mta-analysis-v6.yaml
...
echo "manifests/04-sonataflow_mta-analysis-v6.yaml"
cat manifests/04-sonataflow_mta-analysis-v6.yaml
echo "---"
